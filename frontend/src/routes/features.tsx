...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return <p className="mb-4 font-mono text-xs font-bold text-[#5A8A99]">{children}</p>;
}
...
function FeatureRow({
  ...
  return (
    <section className="mx-auto grid max-w-[1180px] grid-cols-1 items-center gap-10 border-t border-[#D4E8EF] px-6 py-16 sm:px-10 lg:grid-cols-2 lg:gap-16 lg:py-[70px]">
      <div className={reverse ? "lg:order-2" : ""}>
        <Eyebrow>{n}</Eyebrow>
        <h2 className="mb-4 text-[26px] font-bold leading-[1.15] tracking-[-0.02em] text-[#0D3B4C] sm:text-[30px]">{title}</h2>
        <p className="text-[15px] leading-relaxed text-[#5A8A99]">{body}</p>
      </div>
...
function VisualCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-[220px] flex-col justify-center gap-3 rounded-2xl border border-[#D4E8EF] bg-[#F5F9FB] p-7">
      {children}
    </div>
  );
}
  ...
  return (
    <div className="w-full overflow-x-clip bg-white text-[#0D3B4C]">
      ...
      <section className="mx-auto max-w-[900px] px-6 pb-16 pt-24 text-center sm:px-10 sm:pt-28">
        <p className="mb-5 text-[11.5px] font-bold uppercase tracking-[0.16em] text-[#5A8A99]">Features</p>
        <h1 className="text-[38px] font-bold leading-[1.08] tracking-[-0.03em] text-[#0D3B4C] sm:text-5xl lg:text-[60px] lg:leading-[1.04] lg:tracking-[-0.035em]">
          Every stage of payroll-to-invoice, automated.
        </h1>
        <p className="mx-auto mt-6 max-w-[560px] text-base leading-relaxed text-[#5A8A99] sm:text-[17px]">
        ...
        visual={
          <VisualCard>
            {[
              ...
              <div
                key={f.name}
                className="flex items-center gap-3 rounded-[10px] border border-[#D4E8EF] bg-white px-4 py-3"
              >
                <div
                  className="h-2 w-2 shrink-0 rounded-full"
                  style={{ background: "linear-gradient(135deg, #00B4D8, #2DC653)" }}
                />
                <span className="truncate text-[13px] font-semibold text-[#0D3B4C]">{f.name}</span>
                <span className="ml-auto shrink-0 rounded-full bg-[#E8F5EC] px-2.5 py-0.5 text-[11px] font-semibold text-[#2D7A42]">
                  Ingested
                </span>
              </div>
            ))}
            <p className="mt-1 font-mono text-[11.5px] text-[#5A8A99]">polling gmail · every 30s</p>
          </VisualCard>
        }
      />
      ...
      <FeatureRow
        ...
        reverse
        visual={
          <div className="grid h-[220px] grid-cols-2 gap-3 rounded-2xl border border-[#D4E8EF] bg-[#F5F9FB] p-7">
            {["PDF", "Excel / CSV", "Images", "Email body"].map((label) => (
              <div
                key={label}
                className="flex flex-col items-center justify-center gap-1.5 rounded-[10px] border border-[#D4E8EF] bg-white p-4"
              >
                <div
                  className="h-7 w-7 rounded-lg"
                  style={{ background: "linear-gradient(135deg, #00B4D8, #2DC653)" }}
                />
                <span className="text-xs font-semibold text-[#0D3B4C]">{label}</span>
              </div>
            ))}
          </div>
        }
        ...
        visual={
          <VisualCard>
            <div className="flex items-center gap-3.5">
              <div className="flex-1 rounded-[10px] border border-[#D4E8EF] bg-white p-3.5 font-mono text-[11.5px] leading-snug text-[#5A8A99]">
                EmpCd Ov3rt1me
                <br />
                0O77 l2.5
              </div>
              <span className="text-base text-[#5A8A99]">→</span>
              <div
                className="flex-1 rounded-[10px] p-3.5 font-mono text-[11.5px] leading-snug text-white"
                style={{ background: "linear-gradient(135deg, #0D6E8A, #0D3B4C)" }}
              >
                EmpCode Overtime
                <br />
                0077 12.5
            ...
            <p className="font-mono text-[11.5px] text-[#5A8A99]">groq · llama-4-scout-17b-instruct</p>
          </VisualCard>
        }
      />
      ...
      <FeatureRow
        ...
        reverse
        visual={
          <div className="overflow-hidden rounded-2xl border border-[#D4E8EF]">
            {[
              ...
              { label: "Overtime Limits", tone: "Info", bg: "#F5F9FB", fg: "#5A8A99" },
            ].map((row, i, arr) => (
              <div
                key={row.label}
                className={`flex items-center justify-between bg-white px-5 py-3.5 ${i < arr.length - 1 ? "border-b border-[#D4E8EF]" : ""}`}
              >
                <span className="text-[13px] font-semibold text-[#0D3B4C]">{row.label}</span>
                <span
          ...
          <VisualCard>
            {[
              ...
              <div key={row.label} className="flex justify-between text-[13px]">
                <span className="text-[#5A8A99]">{row.label}</span>
                <span className="font-mono font-semibold text-[#0D3B4C]">{row.value}</span>
              </div>
            ))}
            <div className="mt-1.5 flex justify-between border-t border-[#D4E8EF] pt-2.5 text-sm font-bold">
              <span className="text-[#0D3B4C]">Net billable</span>
              <span className="font-mono text-[#0D6E8A]">AED 13,110.50</span>
            </div>
          </VisualCard>
        }
      />
      ...
      {/* Invoice lifecycle */}
      <section className="mx-auto max-w-[1180px] border-t border-[#D4E8EF] px-6 py-16 sm:px-10 sm:py-[70px]">
        <div className="mx-auto mb-12 max-w-[640px] text-center">
          <Eyebrow>06</Eyebrow>
          <h2 className="mb-4 text-[26px] font-bold leading-[1.15] tracking-[-0.02em] text-[#0D3B4C] sm:text-[30px]">
            Invoice generation & lifecycle
          </h2>
          <p className="text-[15px] leading-relaxed text-[#5A8A99]">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {["Draft", "Sent", "Paid"].map((s, i) => (
            <div key={s} className="flex items-center gap-3">
              {i > 0 && <span className="text-[#5A8A99]">→</span>}
              <div className="rounded-full bg-[#0D6E8A] px-6 py-3 text-[13.5px] font-semibold text-white">{s}</div>
            </div>
          ))}
          {["Overdue", "Void"].map((s) => (
            <div key={s} className="flex items-center gap-3">
              <span className="text-[#5A8A99]">/</span>
              <div className="rounded-full border-[1.5px] border-[#D4E8EF] px-6 py-3 text-[13.5px] font-semibold text-[#0D3B4C]">
                {s}
              </div>
            </div>
          ))}
      ...
      <section className="bg-[#0D3B4C] px-6 py-20 text-white sm:px-10 sm:py-24">
        <div className="mx-auto grid max-w-[1180px] grid-cols-1 gap-px overflow-hidden rounded-2xl border border-[#1A4A5A] bg-[#1A4A5A] md:grid-cols-2">
          <div className="bg-[#0D3B4C] p-9 sm:p-11">
            <p className="mb-4 font-mono text-xs font-bold text-[#5A8A99]">07</p>
            <h3 className="mb-3.5 text-2xl font-bold tracking-[-0.02em]">Finance Command Agent</h3>
            <p className="text-[14.5px] leading-relaxed text-[#7AABB8]">
          ...
          <div className="bg-[#0D3B4C] p-9 sm:p-11">
            <p className="mb-4 font-mono text-xs font-bold text-[#5A8A99]">08</p>
      ...
      {/* Audit trail */}
      <section className="mx-auto max-w-[900px] border-b border-[#D4E8EF] px-6 py-20 text-center sm:px-10 sm:py-24">
        <p className="mb-4 font-mono text-xs font-bold text-[#5A8A99]">09</p>
        <h2 className="mb-4 text-[26px] font-bold tracking-[-0.02em] text-[#0D3B4C] sm:text-[30px]">Immutable audit trail</h2>
        <p className="text-[15px] leading-relaxed text-[#5A8A99]">
      ...
      <section className="px-6 py-24 text-center sm:px-10 sm:py-28">
        <h2 className="mb-5 text-[32px] font-bold tracking-[-0.03em] text-[#0D3B4C] sm:text-[42px]">Ready to see it on your own data?</h2>
        <Link
          to="/contact"
